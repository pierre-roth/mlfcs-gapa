...
\begin{abstract}
Deep reinforcement-learning market makers are often trained with hybrid rewards that mix dampened profit-and-loss, transaction advantage, and inventory penalties. These terms work, but they bury the designer's intent inside indistinct scalar coefficients that interact indirectly and must be re-tuned per market. We revisit the Attn-LOB / C-PPO market maker of \citet{guo2023mm} and ask whether the behavioural prior can instead come from a classical quoting model. We propose \softas: PPO with pure profit as the economic reward and a soft action-space regularizer toward a per-stock Avellaneda--Stoikov (AS) teacher fitted on training data. This is a control-prior regularizer, not a reward heuristic: it still shapes the objective, but through an interpretable economic model whose parameters can be calibrated, logged, and stress-tested. On a synthetic, paper-shaped replication benchmark, \softas{} wins $93\%$ of 15 paired stock--seed runs against both controls (paired $\Delta\mathrm{PnL}=15.10$ over \cppo{}, $95\%$ CI $[9.19,21.01]$, $p=8.1\times10^{-5}$; $\Delta\mathrm{PnL}=9.50$ over \profitppo{}, $p=1.9\times10^{-4}$), corresponding to a $\sim$$50\%$ and $\sim$$21\%$ gain in spread-normalized PnL. Because the prior lives in the action space we can read it directly: the policy's mean action gap to the teacher falls from $0.468$ to $0.230$, showing that the regularizer moves quoting behavior, not just terminal PnL. The gain is not free, \softas{} quotes tighter, fills more, carries more inventory, and degrades faster under high latency. On this benchmark, an AS control prior is a cleaner and more inspectable way to steer a deep market maker than hybrid reward shaping.


%On a synthetic, paper-shaped replication benchmark, the selected soft-AS policy improves mean episode PnL over reward-shaped \cppo{} by $15.10$ across 15 stock--seed pairs ($95\%$ CI $[9.19,21.01]$, $p=8.1\times10^{-5}$) and over \profitppo{} by $9.50$ ($95\%$ CI $[5.45,13.56]$, $p=1.9\times10^{-4}$). It also stays much closer to the AS teacher in action space. The tradeoff is visible: \softas{} quotes tighter, fills more, and carries more inventory, especially under high latency. In this benchmark, AS regularization gives a cleaner and more inspectable way to steer a deep market maker than hybrid reward shaping.
\end{abstract}
...
\item We calibrate one AS teacher per stock from the training window, getting a behavioural prior that is inspectable, loggable, and steerable through economically meaningful knobs ($\gamma,\kappa,\lambda$).
\item On a matched synthetic replication of \citet{guo2023mm} with five seeds per stock, we show that \softas{} improves spread-normalized PnL and Sharpe over both reward-shaped \cppo{} and a profit-only control, and we add teacher-divergence, latency, and decision-trace diagnostics that show the prior steers quoting behaviour directly and locate where it breaks.
\end{itemize}
...
\paragraph{Metrics.}
Our primary metric is spread-normalized PnL, $\mathrm{ND\text{-}PnL}=\mathrm{PnL}/(\overline{s}+10^{-7})$, where $\mathrm{PnL}=V_T-V_0$ is the change in marked-to-market value over an episode and $\overline{s}$ is the mean quoted spread. Dividing by $\overline{s}$ makes performance comparable across stocks whose price levels, and therefore absolute spreads and PnL differ substantially; it counts roughly how many spreads the agent captured. We also report raw PnL, Sharpe computed on event-to-event marked-value increments $\Delta V_t$ as $\sqrt{n}\,\overline{\Delta V}/\mathrm{sd}(\Delta V)$ without annualization, and behavioural quantities (mean absolute inventory, mean quoted spread, fills per episode).

\paragraph{Significance.}
Paired tests compare two agents on the same stock and seed before pooling differences across stocks. We report the mean paired difference, a $95\%$ confidence interval, and the paired win rate over the 15 units, but treat the raw PnL magnitude as secondary to ND-PnL and the per-stock breakdown. Direct AS has no training seed, so we evaluate it on the same stock, day, episode-start, and replay-seed schedule and pair it with each of the five learned-policy seed indices. Because the units cluster in three stocks rather than 15 independent draws, we read the pooled test alongside the per-stock breakdown in Appendix~\ref{app:tables} rather than treating the 15 as independent.
...
Our primary comparison is the paired test together with ND-PnL, because the paired test holds the stock and seed fixed while ND-PnL removes most of the price-scale effect across stocks. Against the reward-shaped \cppo{} baseline, \softas{} improves per-unit PnL by $15.10$ ($95\%$ CI $[9.19,21.01]$, $p=8.1\times10^{-5}$) and wins on 14 of 15 units; against \profitppo{}, which already removes the hybrid reward, it improves by $9.50$ ($p=1.9\times10^{-4}$) and again wins on 14 of 15 (Table~\ref{tab:paired}). The second comparison is the more informative one: \profitppo{} has stripped out the old reward entirely, so the gain over it is attributable to the calibrated AS prior rather than to dropping the hybrid terms.

The aggregate metrics in Table~\ref{tab:main} tell the same story in scale-comparable form. On ND-PnL, which counts how many spreads each agent captures and is therefore comparable across stocks, \softas{} reaches $1223.9$ against $812.1$ for \cppo{} and $1010.5$ for \profitppo{}--roughly a $50\%$ and $21\%$ improvement--and it also leads on Sharpe ($2.54$ vs.\ $2.07$ and $2.29$). Raw mean PnL is reported for completeness but should be read with care: because the three stocks differ in price level, the mean PnL is dominated by the high-price stock and is not comparable across instruments.

That dominance does not, however, carry into the improvement. Figure~\ref{fig:perstock} and Appendix~\ref{app:tables} break the gain down by stock: \softas{} improves ND-PnL and Sharpe over \cppo{} on the low-price stock 000001 and the mid-price stock 002415, and remains competitive on the liquid high-price stock 000858, where the profit-only control is the strongest on ND-PnL. The gain is therefore spread across stocks rather than a single-stock artifact.
...
diagnostics in Section~\ref{sec:res-behaviour} explain this pattern:
...
action prior. The motivation was practical: hybrid rewards encode useful behavioural intentions but spreads them across coupled coefficients which are hard to interpret, to tune, and to
...
setup, \softas{} improves spread-normalized PnL by roughly $50\%$
over reward-shaped \cppo{} and by $21\%$ over a profit-only control
...
policy stays close to the AS teacher but moves away from it when the data reward a different quote, which is the mechanism by which it outperforms the teacher and the unregularized baseline.

The improvement is not free. \softas{} quotes tighter, fills more
...
non-negligible communication or computation delays. These are direct consequences of the method's strength: a prior that encourages participation will always carry
