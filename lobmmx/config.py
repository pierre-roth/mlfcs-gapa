...
class ExperimentConfig:
    ...
    pretrain_task_mode: str = "mm_multitask"
    pretrain_spread_alpha_ticks: float = 0.25
    pretrain_flow_alpha: float = 100.0
    ...
    max_inventory_skew_bps: float = 3.0
    reward_scale_mode: str = "spread"
    maker_rebate_per_share: float = 0.0013
    taker_fee_per_share: float = 0.0030
    fill_model: str = "queue"  # "legacy" = old vol/(vol+depth), "queue" = queue-position-aware
    queue_position: str = "back"  # "back" = join back of queue, "uniform" = random position
    ...
    rv_windows_s: list[int] = field(default_factory=lambda: [1, 5, 15, 60])
    rsi_windows_s: list[int] = field(default_factory=lambda: [1, 5, 15, 60])
    osi_windows_s: list[int] = field(default_factory=lambda: [1, 5, 15, 60])
    ppo_epochs: int = 10
    ...
    def apply_mode_defaults(self) -> "ExperimentConfig":
        ...
        self.ppo_checkpoint_every = max(int(self.ppo_checkpoint_every), 0)
        if self.pretrain_eval_samples_per_day is None:
...
class RLTrainConfig(ExperimentConfig):
    algorithm: str = "ppo"
    state_mode: str = "full"
    reward_mode: str = "trade_inventory"
