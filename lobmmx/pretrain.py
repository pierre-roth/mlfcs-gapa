...
from .models import PretrainMultiTask, build_backbone
...
def _evaluate_classifier(
    model: PretrainMultiTask,
    loader: DataLoader,
    device: str,
) -> dict[str, float | int | list[int]]:
    task_names = ["mid", "spread", "flow"]
    preds = {name: [] for name in task_names}
    targets = {name: [] for name in task_names}
    model.eval()
    with torch.no_grad():
        for lob, labels in loader:
            outputs = model(lob.to(device))
            for idx, name in enumerate(task_names):
                preds[name].extend(outputs[idx].argmax(dim=-1).cpu().tolist())
                targets[name].extend(labels[:, idx].tolist())
    if not targets["mid"]:
        return {
            "precision": 0.0,
            "recall": 0.0,
    ...
    for name in task_names:
        precision, recall, f1, _ = precision_recall_fscore_support(targets[name], preds[name], average="macro", zero_division=0)
        accuracy = float(sum(int(p == t) for p, t in zip(preds[name], targets[name])) / len(targets[name]))
        if name == "mid":
            metrics["precision"] = float(precision)
            metrics["recall"] = float(recall)
            metrics["f1"] = float(f1)
            metrics["accuracy"] = accuracy
            metrics["target_class_counts"] = [int(value) for value in np.bincount(np.asarray(targets[name], dtype=np.int64), minlength=3)]
            metrics["predicted_class_counts"] = [int(value) for value in np.bincount(np.asarray(preds[name], dtype=np.int64), minlength=3)]
            metrics["confusion_matrix"] = confusion_matrix(targets[name], preds[name], labels=[0, 1, 2]).astype(int).tolist()
        else:
            metrics[f"{name}_f1"] = float(f1)
            metrics[f"{name}_accuracy"] = accuracy
    return {
        **metrics,
        "samples": int(len(targets["mid"])),
    }
...
def run_pretrain(config: PretrainConfig) -> dict[str, dict[str, float | str]]:
    ...
    try:
        for symbol in config.symbols:
            ...
            model = PretrainMultiTask(backbone).to(config.device)
            ...
            for epoch in range(start_epoch, config.pretrain_epochs):
                ...
                for lob, labels in train_loader:
                    ...
                    optimizer.zero_grad()
                    logits_mid, logits_spread, logits_flow = model(lob)
                    loss = (
                        criterion(logits_mid, labels[:, 0])
                        + criterion(logits_spread, labels[:, 1])
                        + criterion(logits_flow, labels[:, 2])
                    ) / 3.0
                    loss.backward()
                    optimizer.step()
                ...
                epoch_record = {
                    ...
                    "f1": f1,
                    "spread_f1": float(val_metrics.get("spread_f1", 0.0)),
                    "flow_f1": float(val_metrics.get("flow_f1", 0.0)),
