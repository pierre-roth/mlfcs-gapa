...
from lobmmx.config import ExperimentConfig, PretrainConfig, RLTrainConfig, SuiteConfig
...
def _resolve_runner(kind: str):
    if kind == "pretrain":
        from lobmmx.pretrain import run_pretrain

        return PretrainConfig, run_pretrain
    if kind == "train":
        from lobmmx.train_rl import run_rl_training

        return RLTrainConfig, run_rl_training
    if kind == "evaluate":
        from lobmmx.evaluate import run_evaluation

        return RLTrainConfig, run_evaluation
    if kind == "report":
        from lobmmx.report import run_report

        return ExperimentConfig, run_report
    if kind == "suite":
        from lobmmx.run_suite import run_suite
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Dispatch Euler batch jobs into the lobmmx experimental entry points.")
