...
def _resolve_runner(kind: str):
    ...
    if kind == "matrix":
        from lobmmsim.learning_matrix import run_learning_matrix

        return LearningMatrixConfig, run_learning_matrix
    if kind == "pretrain":
        from lobmmsim.pretrain import run_pretrain

        return PretrainConfig, run_pretrain
    if kind == "report":
        from lobmmsim.report import run_report

        return RLTrainConfig, run_report
    if kind == "suite":
        from lobmmsim.run_pipeline import run_suite

        return SuiteConfig, run_suite
    if kind == "train":
        from lobmmsim.train_rl import run_rl_training
