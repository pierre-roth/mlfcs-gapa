...
mlfcs-gapa generate-synthetic \
  --output-dir "${SCRATCH}/mlfcs-gapa/data/synthetic-smoke" \
  --days 1 \
  --events-per-day 300 \
  --seed 101
...
path = Path(__import__("os").environ["SCRATCH"]) / "mlfcs-gapa/data/synthetic-smoke/000001/2019-11-01/orderbook.parquet"
...
mlfcs-gapa run-synthetic-baselines \
  --output-dir "${SCRATCH}/mlfcs-gapa/runs/synthetic-baseline-smoke" \
  --days 1 \
  --events-per-day 300 \
  --episode-events 200 \
  --tabular-episodes 3 \
...
path = Path(os.environ["SCRATCH"]) / "mlfcs-gapa/runs/synthetic-baseline-smoke/baseline_metrics.csv"
...
mlfcs-gapa pretrain-synthetic-attn-lob \
  --output-dir "${SCRATCH}/mlfcs-gapa/runs/synthetic-pretrain-smoke" \
  --events 220 \
  --epochs 1 \
  --batch-size 32 \
  --seed 101
...
path = Path(os.environ["SCRATCH"]) / "mlfcs-gapa/runs/synthetic-pretrain-smoke/attn_lob_pretrain_metrics.csv"
metrics = pl.read_csv(path)
assert metrics.height == 1, metrics
assert metrics["model"][0] == "Attn-LOB"
model_path = Path(os.environ["SCRATCH"]) / "mlfcs-gapa/runs/synthetic-pretrain-smoke/attn_lob_pretrain_model.pt"
assert model_path.exists(), model_path
print("pretrain smoke rows", metrics.height)
PY

mlfcs-gapa train-synthetic-ppo \
  --output-dir "${SCRATCH}/mlfcs-gapa/runs/synthetic-ppo-smoke" \
  --events 240 \
  --episode-events 160 \
  --total-timesteps 32 \
  --n-steps 16 \
...
root = Path(os.environ["SCRATCH"]) / "mlfcs-gapa/runs/synthetic-ppo-smoke"
...
mlfcs-gapa train-synthetic-ddqn \
  --output-dir "${SCRATCH}/mlfcs-gapa/runs/synthetic-ddqn-smoke" \
  --events 240 \
  --episode-events 160 \
  --total-timesteps 32 \
  --learning-starts 8 \
...
root = Path(os.environ["SCRATCH"]) / "mlfcs-gapa/runs/synthetic-ddqn-smoke"
...
mlfcs-gapa run-synthetic-latency-baselines \
  --output-dir "${SCRATCH}/mlfcs-gapa/runs/synthetic-latency-smoke" \
  --latencies 1,5 \
  --days 1 \
  --events-per-day 300 \
  --episode-events 200 \
...
mlfcs-gapa summarize-metrics \
  "${SCRATCH}/mlfcs-gapa/runs/synthetic-latency-smoke/latency_metrics.csv" \
  --output-path "${SCRATCH}/mlfcs-gapa/runs/synthetic-latency-smoke/summary_metrics.csv"

mlfcs-gapa plot-decision-trace \
  "${SCRATCH}/mlfcs-gapa/runs/synthetic-latency-smoke/latency_trades.parquet" \
  --output-path "${SCRATCH}/mlfcs-gapa/runs/synthetic-latency-smoke/decision_trace.png"

mlfcs-gapa plot-synthetic-attention \
  --output-path "${SCRATCH}/mlfcs-gapa/runs/synthetic-latency-smoke/attention_heatmap.png" \
  --events 220 \
  --index 80 \
  --seed 101
...
root = Path(os.environ["SCRATCH"]) / "mlfcs-gapa/runs/synthetic-latency-smoke"
...
mlfcs-gapa benchmark-runtime-synthetic \
  --output-path "${SCRATCH}/mlfcs-gapa/runs/runtime-smoke/runtime_metrics.csv" \
  --events 220 \
  --episode-events 140 \
  --train-timesteps 16 \
  --device cpu \
...
path = Path(os.environ["SCRATCH"]) / "mlfcs-gapa/runs/runtime-smoke/runtime_metrics.csv"
metrics = pl.read_csv(path)
assert metrics.height == 7, metrics
