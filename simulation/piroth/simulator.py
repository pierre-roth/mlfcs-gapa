...
class AgentBasedLOB:
    ...
    def _ensure_side_depth(self, side: str, depth: int = 10) -> None:
        ...
        for price in missing_prices:
            self._add_limit(
                side,
                price,
                self._draw_size(self.profile.depth_scale),
    ...
    def _event_weights(self) -> tuple[list[str], np.ndarray]:
        ...
        weights = np.asarray(
            [
                ...
                self.profile.informed_taker_rate * np.exp(0.55 * signal_edge) * (0.88 + 0.12 * vol_scale),
                self.profile.informed_taker_rate * np.exp(-0.55 * signal_edge) * (0.88 + 0.12 * vol_scale),
                # Competing MMs add: very slight retreat during strong signal
                self.profile.maker_add_rate * np.exp(-0.12 * signal_edge + 0.14 * max(-imbalance, 0.0)) * np.exp(-0.04 * abs(signal_edge)),
                self.profile.maker_add_rate * np.exp(0.12 * signal_edge + 0.14 * max(imbalance, 0.0)) * np.exp(-0.04 * abs(signal_edge)),
                # Competing MMs cancel: moderately stronger signal response (was 0.16, now 0.22)
                self.profile.maker_cancel_rate * np.exp(0.22 * signal_edge + 0.10 * flow_term) * (0.92 + 0.08 * vol_scale),
                self.profile.maker_cancel_rate * np.exp(-0.22 * signal_edge - 0.10 * flow_term) * (0.92 + 0.08 * vol_scale),
    ...
    def _market_order(self, side: str, taker_agent: str) -> tuple[list[TradeRecord], float]:
        ...
        # Touch replenishment (keeps book from totally collapsing)
