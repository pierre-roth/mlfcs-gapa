...
class ContinuousMarketEnv:
    ...
    def _agent_state(self, idx: int) -> np.ndarray:
        ...
        # Spread and depth features (useful microstructure signals)
        spread_ticks = float(self.day.spread[idx]) / max(self.config.tick_size, 1e-8)
        spread_norm = min(spread_ticks / 10.0, 1.0)  # normalized to [0,1]
        # Top-of-book imbalance from the LOB snapshot
        ask1_vol = float(self.day.ask.iloc[idx].get("ask1_volume", 0.0)) if idx < len(self.day.ask) else 0.0
        bid1_vol = float(self.day.bid.iloc[idx].get("bid1_volume", 0.0)) if idx < len(self.day.bid) else 0.0
        imbalance = (bid1_vol - ask1_vol) / max(bid1_vol + ask1_vol, 1e-8)
        # Latent diagnostics available from the simulator
        vol_state = float(self.day.latent.iloc[idx].get("vol_state", 1.0)) if "vol_state" in self.day.latent.columns else 1.0
        ...
        return np.asarray(
            ...
            + [vol_norm] * 3
            + [cash_norm] * 3
            + [float(abs(inv_scaled))] * 2  # abs inventory (risk level)
            + [float(np.sign(inv_scaled) * spread_norm)] * 2,  # directional spread
            dtype=np.float32,
        )
        ...
        self.logs.append(
            {
                ...
                "vol_state": float(self.day.latent.iloc[event_idx].get("vol_state", 1.0)) if "vol_state" in self.day.latent.columns else 1.0,
