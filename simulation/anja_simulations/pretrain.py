...
def run_pretrain(config: PretrainConfig) -> dict[str, dict[str, float]]:
    ...
    for symbol in config.symbols:
        ...
        opt = Adam(model.parameters(), lr=config.pretrain_lr, weight_decay=config.pretrain_weight_decay)
        # Class weights: inverse frequency, normalized to mean 1 — handles imbalance
        # from pretrain_alpha thresholding (flat class is typically rare).
        train_labels = train_ds.tensors[1].numpy()
        class_counts = np.bincount(train_labels, minlength=3).astype(np.float64)
        class_weights_raw = 1.0 / np.clip(class_counts, 1.0, None)
        class_weights = class_weights_raw / class_weights_raw.mean()
        class_weights_t = torch.tensor(class_weights, dtype=torch.float32, device=config.device)
        criterion = nn.CrossEntropyLoss(weight=class_weights_t, label_smoothing=config.pretrain_label_smoothing)
        print(f"  [pretrain] class counts: {class_counts.astype(int).tolist()}, weights: {class_weights.round(3).tolist()}")
