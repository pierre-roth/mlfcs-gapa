...
class ContinuousMarketEnv:
    def __init__(self, day: DayData, config: TrainConfig) -> None:
        ...
        # Cache for fast latent lookup (avoid per-step .iloc)
        self._vol_state_arr: np.ndarray | None = None
        self._latent_alpha_arr: np.ndarray | None = None
        self._regime_shift_arr: np.ndarray | None = None
        self._event_actor_arr: np.ndarray | None = None
        self._maker_agent_arr: np.ndarray | None = None
        self._queue_pressure_arr: np.ndarray | None = None
        self._top_imbalance_arr: np.ndarray | None = None
    ...
    def _agent_state(self, idx: int) -> np.ndarray:
        ...
        # Spread features (use precomputed numpy arrays — much faster than .iloc)
        spread_ticks = float(self.day.spread[idx]) / max(self.config.tick_size, 1e-8)
        spread_norm = min(spread_ticks / 10.0, 1.0)
        # Top-of-book imbalance from the LOB snapshot (ask1_vol at [1], bid1_vol at [3])
        if idx < len(self.day.lob):
            row = self.day.lob[idx]
            ask1_vol = float(row[1])
            bid1_vol = float(row[3])
        else:
            ask1_vol = bid1_vol = 0.0
        imbalance = (bid1_vol - ask1_vol) / max(bid1_vol + ask1_vol, 1e-8)
        # Latent diagnostics — cached for speed
        if self._vol_state_arr is None:
            self._vol_state_arr = (
                self.day.latent["vol_state"].to_numpy(dtype=np.float32)
                if "vol_state" in self.day.latent.columns
                else np.ones(len(self.day.midprice), dtype=np.float32)
            )
        vol_state = float(self._vol_state_arr[idx]) if idx < len(self._vol_state_arr) else 1.0
        vol_norm = min(vol_state / 3.0, 1.0)
        # Cash-to-turnover ratio (profitability signal)
        cash_ratio = self.cash / max(self.turnover, 1e-8) if self.turnover > 0 else 0.0
        cash_norm = float(np.clip(cash_ratio, -1.0, 1.0))
        return np.asarray(
            [inv_scaled] * 4
            + [remaining] * 4
            + [spread_norm] * 3
            ...
            + [vol_norm] * 3
            + [cash_norm] * 3
            + [float(abs(inv_scaled))] * 2
            + [float(np.sign(inv_scaled) * spread_norm)] * 2,
            dtype=np.float32,
        )
    ...
    def _ensure_log_caches(self) -> None:
        """Lazily populate numpy caches from latent DataFrame (once per env)."""
        if self._latent_alpha_arr is None:
            n = len(self.day.midprice)
            latent = self.day.latent
            self._latent_alpha_arr = latent["latent_alpha"].to_numpy(dtype=np.float32) if "latent_alpha" in latent.columns else np.zeros(n, dtype=np.float32)
            self._regime_shift_arr = latent["regime_shift"].to_numpy(dtype=np.int32) if "regime_shift" in latent.columns else np.zeros(n, dtype=np.int32)
            self._event_actor_arr = latent["event_actor"].astype(str).to_numpy() if "event_actor" in latent.columns else np.array([""] * n)
            self._maker_agent_arr = latent["maker_agent"].astype(str).to_numpy() if "maker_agent" in latent.columns else np.array([""] * n)
            self._queue_pressure_arr = latent["queue_pressure"].to_numpy(dtype=np.float32) if "queue_pressure" in latent.columns else np.zeros(n, dtype=np.float32)
            self._top_imbalance_arr = latent["top_imbalance"].to_numpy(dtype=np.float32) if "top_imbalance" in latent.columns else np.zeros(n, dtype=np.float32)
            if self._vol_state_arr is None:
                self._vol_state_arr = latent["vol_state"].to_numpy(dtype=np.float32) if "vol_state" in latent.columns else np.ones(n, dtype=np.float32)

    def step(self, action: np.ndarray | dict[str, float]) -> tuple[Observation, float, bool, dict[str, float]]:
        self._ensure_log_caches()
        ...
        self.logs.append(
            {
                ...
                "latent_alpha": float(self._latent_alpha_arr[event_idx]),
                "regime_shift": int(self._regime_shift_arr[event_idx]),
                "event_actor": str(self._event_actor_arr[event_idx]),
                "maker_agent": str(self._maker_agent_arr[event_idx]),
                "queue_pressure": float(self._queue_pressure_arr[event_idx]),
                "top_imbalance": float(self._top_imbalance_arr[event_idx]),
                "vol_state": float(self._vol_state_arr[event_idx]),
                "spread_ticks": float(self.day.spread[event_idx]) / max(self.config.tick_size, 1e-8),
                "reward": float(reward),
            }
        )
        self.cursor += 1
