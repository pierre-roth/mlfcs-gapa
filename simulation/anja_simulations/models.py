...
class AttnLOB(nn.Module):
    def __init__(
        ...
        self.attn = nn.MultiheadAttention(self.attn_dim, num_heads=num_heads, batch_first=True, dropout=attn_dropout)
        self.pre_output_dropout = nn.Dropout(attn_dropout)
    ...
    def features(self, lob: torch.Tensor) -> torch.Tensor:
        ...
        attn_out = self.pre_output_dropout(attn_out.squeeze(1))
...
class SharedStateEncoder(nn.Module):
    def __init__(self, backbone: nn.Module, flat_dim: int, hidden_dim: int = 128) -> None:
        super().__init__()
        self.backbone = backbone
        self.trunk = nn.Sequential(
            nn.Linear(backbone.output_dim + flat_dim, hidden_dim),
            nn.LeakyReLU(0.01),
    ...
    def forward(self, lob: torch.Tensor, flat: torch.Tensor) -> torch.Tensor:
        return self.trunk(torch.cat([self.backbone.features(lob), flat], dim=-1))


class ContinuousActorCritic(nn.Module):
    def __init__(self, encoder: SharedStateEncoder, action_dim: int = 2) -> None:
        super().__init__()
    ...
    def dist_value(self, lob: torch.Tensor, flat: torch.Tensor) -> tuple[Beta, torch.Tensor]:
