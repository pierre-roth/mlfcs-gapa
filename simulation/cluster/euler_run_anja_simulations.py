...
from anja_simulations.config import GenerateConfig, PretrainConfig, ReportConfig, SuiteConfig, SweepConfig, TrainConfig
...
def _resolve_runner(kind: str):
    if kind == "generate":
        from anja_simulations.simulator import generate_dataset

        return GenerateConfig, generate_dataset
    if kind == "pretrain":
        from anja_simulations.pretrain import run_pretrain

        return PretrainConfig, run_pretrain
    if kind == "train":
        from anja_simulations.train import run_train

        return TrainConfig, run_train
    if kind == "report":
        from anja_simulations.report import run_report

        return ReportConfig, run_report
    if kind == "suite":
        from anja_simulations.run_suite import run_suite

        return SuiteConfig, run_suite
    if kind == "sweep":
        from anja_simulations.sweep import run_sweep
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Dispatch Euler jobs into the anja_simulations pipeline.")
