...
def test_spread_distribution_stays_in_learnable_range() -> None:
    cfg = GenerateConfig(mode="full", symbols=["000001"], events_per_day={"000001": 5_000}, seed=17, device="cpu")
    from piroth.simulator import generate_day_frames

    frames = generate_day_frames("000001", 0, cfg)
    spread_ticks = ((frames["price"]["ask1_price"] - frames["price"]["bid1_price"]) / cfg.tick_size).round()
    assert 1.1 <= float(spread_ticks.mean()) <= 2.4
    assert 0.02 <= float((spread_ticks >= 3).mean()) <= 0.35
