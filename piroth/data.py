...
def _paper_normalize_lob(ask: pd.DataFrame, bid: pd.DataFrame, midprice: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    raw_rows = []
    norm_rows = []
...
def load_day(symbol: str, day: str, config: ExperimentConfig) -> DayData:
    ...
    spread = ask1 - bid1
    lob, normalized_lob = _paper_normalize_lob(ask, bid, midprice)
...
def load_splits(config: ExperimentConfig, symbol: str) -> dict[str, list[DayData]]:
    ...
    all_days = [load_day(symbol, (pd.Timestamp("2019-11-01") + pd.tseries.offsets.BDay(i)).strftime("%Y%m%d"), config) for i in range(config.num_days)]
